...
class TestAssessApi(object):
    ...
    def test_get_NewsItem_unauth(self, client, news_item_data, db_session):
        """
    ...
    def test_get_NewsItem_auth(self, client, news_item_data, db_session, auth_header):
        """
    ...
    def test_get_NewsItemAggregatesTags_unauth(self, client, news_item_data, db_session):
        """
    ...
    def test_get_NewsItemAggregatesTags(self, client, news_item_data, db_session, auth_header):
        """
        This test queries the NewsItemsAggregatesTags UNauthenticated.
        It expects "not authorized"
        """

        from core.model.news_item import NewsItemAggregate, NewNewsItemDataSchema, NewsItem, NewsItemTag
        from core.model.osint_source import OSINTSource
        from shared.schema.osint_source import OSINTSourceExportSchema
        from core.model.user import User
        import datetime
        ...
        tag1 = NewsItemTag("foo", "some-tag-type")
        tag2 = NewsItemTag("bar", "some-tag-type")
        nia.tags.append(tag1)
        nia.tags.append(tag2)
        db_session.add(nia)
        db_session.commit()
